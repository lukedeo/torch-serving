...
/**
 * Converts a torch::Tensor object into a JSON representation, specifically by
 * flattening out the tensor values (equivalent to np.ravel in Python) and
 * storing both the data type (int16, float32, etc.) and the shape of the tensor
 * in order to reconstruct fully later.
 *
 * @param tensor A torch::Tensor object to convert
 * @return A json::json object containing the fields 'shape' (containing the
 * integer-list representation of the shape), 'value' (containing a list of
 * numbers representing the flattened tensor), and 'data_type' (containing the
 * numeric type, i.e., int16, float32, etc.)
 */
...
/**
 * Converts a torch::Scalar object to a JSON representation.
 * @param  A torch::IValue object to convert into a torch::Scalar and then to a
 * JSON object
 * @return A json::json object containing the fields 'value' (containing the
 * number itself), and 'data_type' (containing the numeric type, i.e., int16,
 * float32, etc.)
 */
json::json ScalarToJson(const torch::jit::IValue &torch_value) {
  auto scalar = torch_value.toScalar();
  auto dtype =
      torch::typeMetaToScalarType(torch::scalar_to_tensor(scalar).dtype());
